...
let initialBlocs = Array.from(document.querySelectorAll(".bloc"))
let toutLesBlocs = initialBlocs.slice(0)
...
function resetGame() {
    ...
    toutLesBlocs = initialBlocs.slice(0);
...
//---------------------------------- FONCTION DETERMINER WIN OU LOOSE
...
function winLoose() {
    ...
    for (let colonne = 0; colonne < 3; colonne++) {
        const colonneElements = [Ligne1[colonne], Ligne2[colonne], Ligne3[colonne]];
...
// Verification pour le draw (egalité)
...
blocsLigne1.forEach(element => {
    element.addEventListener("click",function () {
        ...
        let caseRandom = toutLesBlocs[Math.floor(Math.random() * toutLesBlocs.length)];
